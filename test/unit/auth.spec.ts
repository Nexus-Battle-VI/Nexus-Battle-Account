...
const contextFor = (request: FakeRequest, metadata: Record<string, unknown> = {}) => {
  ...
  const context = {
    ...
    getHandler: () => handler,
    // El guard solo usa esta referencia como clave del reflector, nunca la
    // instancia: basta con que sea estable dentro de la prueba.
    getClass: () => contextFor,
...
describe('JwtAuthGuard', () => {
  ...
  /**
   * Esta es la distincion que importa: si el JWKS no se puede alcanzar, el
   * testimonio no es invalido, es que NO SE HA PODIDO COMPROBAR. Responder 401
   * afirmaria algo que no se sabe, y ademas ocultaria una caida de red detras
...
describe('RolesGuard', () => {
   ...
   * fallo de orden de los guards en una ruta abierta.
   */
