...
- El servicio trata correos electrónicos, nombres visibles y el país opcional declarado (`countryCode`), que son datos personales. La observabilidad registra el dominio del correo, nunca la dirección completa ni el país. El país se incluye en la proyección/exportación administrativa con sus controles de acceso existentes; se puede borrar con `PATCH /api/accounts/me` y `countryCode: null`. Ver [contrato del perfil](docs/profile-country.md).
